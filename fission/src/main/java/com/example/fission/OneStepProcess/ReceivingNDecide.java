...
//TODO
    ...
    public static List<Agent> decideToHelp(Agent receiver, List<Agent> agent){
        ...
        if (agent.size()==0 || receiver.getMaxHelpFissionPeople()==0){
            return helping;
        }
...
//        }
        //return result;
        List<Agent> result = (agent.size()<=maxNumHelping?agent:agent.subList(0,maxNumHelping));
        receiver.setMaxHelpFissionPeople(receiver.getMaxHelpFissionPeople()-result.size());
        //System.out.println("helped "+  result.size() +" people");
        return  result;
    }
